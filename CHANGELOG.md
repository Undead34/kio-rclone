...
- KIO-FUSE integration keeps random-access `FileJob` capabilities disabled
  for `rclone:/`, so editor reads and writes stay in KIO-FUSE's local cache
  and reach this worker through one complete `get`/`put` transaction.
- Virtual exports reported with `Size: -1` are downloaded once for an exact
  `stat()` size and reused by `get()` in the same worker.
